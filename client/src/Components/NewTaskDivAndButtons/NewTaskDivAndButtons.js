	...
	handleClick = value => {
		if (value) {
			if (value === 'Start Button') {
				this.setState(state => {
					return (
						state.currentButtons.splice(0, 1, [
							'East Zone',
							'West Zone'
						]) &&
						state.currentButtons.splice(1, 1, []) &&
						state.currentButtons.splice(2, 1, []) &&
						state.currentButtons.splice(3, 1, []) &&
						state.currentButtons.splice(4, 1, []) &&
						state.currentButtons.splice(5, 1, []) &&
						state.currentButtons.splice(6, 1, [])
					);
				});
